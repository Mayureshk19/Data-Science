#Win Probablity Prediction Over by Over
...
match_df = match_df.loc[match_df.season==2016,:]
...
#splitting the data and keeping the final match as our validation sample
...
val_df = train_df.loc[train_df.match_id == 577, :]
dev_df = train_df.loc[train_df.match_id != 577, :]
...
#defining the function to create a model using  XgBoost
...
#building the model and getting the predictions for the final match
...
#run rate is the most important predictor for win
#innings score is the second
#target score being the third
...
ax2.grid(b = False)
plt.show()
#yellow bars represents the corressponding overs played by SRH  while green is for RCB 
#blue line represents the win probability of SRH at the end of each over.


#using the number of runs scored in that over in place of overall runs.
fig, ax1 = plt.subplots(figsize = (12,6))
ax2 = ax1.twinx()
labels = np.array(out_df['innings_over'])
ind = np.arange(len(labels))
